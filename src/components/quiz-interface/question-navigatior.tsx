...
export default function QuestionNavigator({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "answered":
        return "bg-green-500 border-green-600 text-white hover:bg-green-600 hover:text-white";
      case "flagged":
        return "bg-yellow-500 border-yellow-600 text-white hover:bg-yellow-600 hover:text-white";
      default:
      ...
      {groupedData[subjectInfo.subjectIndex].questions.map((q, index) => {
        ...
        return (
          <button
            ...
            className={`w-full aspect-square rounded-md text-sm font-semibold border flex items-center justify-center shadow-sm transition-colors duration-200 ${
